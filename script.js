...
// 1. OSCILADOR DE ENTROPIA DE MERCADO (CORRIGIDO)
function calcularEntropiaMercado(dados, periodo = CONFIG.PERIODOS.ENTROPIA) {
  try {
    if (!Array.isArray(dados) || dados.length < periodo * 2) return 0.5;
...
async function analisarMercado() {
  ...
  try {
    ...
    state.leituraEmAndamento = false;
    sincronizarTimer(); // CORREÇÃO CRÍTICA: Reinicia o timer mesmo após erros
  }
}
