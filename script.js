// =============================================
        ...
        function calcularRSICompleto(closes, periodo = CONFIG.PERIODOS.RSI) {
          ...
          // Médias iniciais
          let avgGain = calcularMedia.simples(gains.slice(0, periodo), periodo) || 1;
          let avgLoss = calcularMedia.simples(losses.slice(0, periodo), periodo) || 1;
          
          const rsis = [100 - (100 / (1 + (avgGain / (avgLoss || 1)))];
        ...
        function gerarSinal(indicadores, divergencias, lateral) {
          ...
          if (rsi < CONFIG.LIMIARES.RSI_OVERSOLD && 
              stoch < CONFIG.LIMIARES.STOCH_OVERSOLD && 
          ...
          if (rsi > CONFIG.LIMIARES.RSI_OVERBOUGHT && 
              stoch > CONFIG.LIMIARES.STOCH_OVERBOUGHT && 
              macd.histograma < -0.01 &&
              close < ema50) {
            return "PUT";
          }
        ...
        async function obterDadosTwelveData() {
          try {
            ...
            if (!response.ok) {
              // Rotacionar chave se houver erro
              currentKeyIndex = (currentKeyIndex + 1) % API_KEYS.length;
              throw new Error(`Falha na API: ${response.status} (tentando próxima chave)`);
            }
            
            const data = await response.json();
            
            if (data.status === 'error') {
              // Rotacionar chave se a API retornar erro
              currentKeyIndex = (currentKeyIndex + 1) % API_KEYS.length;
