...
const CONFIG = {
  ...
  PARES: {
    FOREX: "EUR/USD"  // Alterado para EUR/USD
  },
  PERIODOS: {
    RSI: 9,
    STOCH_K: 14,
  ...
  LIMIARES: {
    SCORE_ALTO: 85,
    SCORE_MEDIO: 70,
    RSI_OVERBOUGHT: 70,    // Mais conservador para Forex
    RSI_OVERSOLD: 30,      // Mais conservador para Forex
    STOCH_OVERBOUGHT: 85,
    STOCH_OVERSOLD: 15,
    VARIACAO_LATERAL: 0.002,  // Reduzido para volatilidade Forex
    ATR_LIMIAR: 0.005,        // Reduzido para volatilidade Forex
...
function gerarSinal(indicadores, divergencias, lateral) {
  ...
  // Priorizar tendência forte em Forex
  ...
  const limiteBreakout = variacao * 0.02;  // Mais conservador para Forex
...
function calcularScore(sinal, indicadores, divergencias) {
  ...
  const fatores = {
    ...
    volatilidade: (indicadores.atr / indicadores.close) > 0.008 ? 10 : 0  // Ajustado para Forex
  };
...
function iniciarAplicativo() {
  ...
  container.innerHTML = `
    ...
    <div style="text-align: center; font-size: 14px; opacity: 0.7; padding-top: 15px; border-top: 1px solid #3a3b4a;">
      FOREX - EUR/USD - Análise em tempo real | Atualizado: <span id="ultima-atualizacao">${new Date().toLocaleTimeString()}</span>
    </div>
  `;
