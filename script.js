// =============================================
// CONFIGURAÇÕES GLOBAIS (ATUALIZADAS PARA TWELVE DATA - GBP/NZD)
// =============================================
const state = {
  ultimos: [],
  timer: 60,
...
function calcularVolumeProfile(dados, periodo = CONFIG.PERIODOS.VOLUME_PROFILE) {
  try {
    ...
    const slice = dados.slice(-periodo);
    const buckets = {};
    const precisao = 4; // Aumentado para Forex
...
async function obterDadosForex() {
  try {
    const symbol = encodeURIComponent(CONFIG.PARES.FOREX);
    const interval = '1min';
    const outputsize = 100;
    
    const response = await fetch(
      `${CONFIG.API_ENDPOINTS.TWELVE_DATA}/time_series?symbol=${symbol}&interval=${interval}&outputsize=${outputsize}&apikey=9cf795b2a4f14d43a049ca935d174ebb`
    );
