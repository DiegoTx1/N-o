 // =============================================
        ...
        const CONFIG = {
          ...
          PERIODOS: {
            ...
            ATR: 14,
            SUPERTREND: 7,
            DIVERGENCIA_LOOKBACK: 14, // Aumentado para melhor detecção
            EXTREME_LOOKBACK: 3
          },
          LIMIARES: {
            SCORE_ALTO: 85,
        ...
        function avaliarTendencia(ema5, ema13) {
          ...
          if (forca > 60) { // Limiar reduzido para 60%
            return diff > 0 
        ...
        // =============================================
        // GERADOR DE SINAIS OTIMIZADO PARA CRIPTO (CORRIGIDO)
        // =============================================
        function gerarSinal(indicadores, divergencias, lateral) {
          ...
          // Priorizar tendência forte em cripto (limiar reduzido para 60%)
          ...
          // Breakout em criptomoedas (limite reduzido para 1.5%)
          const variacao = state.resistenciaKey - state.suporteKey;
          const limiteBreakout = variacao * 0.015;
          ...
          // Condições específicas para cripto (limiares ajustados)
        ...
        function atualizarInterface(sinal, score, tendencia, forcaTendencia) {
          if (!state.marketOpen || state.interfaceUpdatePending) return;
          
          state.interfaceUpdatePending = true;
          requestAnimationFrame(() => {
            const comandoElement = document.getElementById("comando");
            if (comandoElement) {
              comandoElement.textContent = sinal;
              comandoElement.className = sinal.toLowerCase() + " pulse";
            ...
            if (scoreElement) {
              ...
              if (score >= CONFIG.LIMIARES.SCORE_ALTO) scoreElement.style.color = '#00cec9';
              else if (score >= CONFIG.LIMIARES.SCORE_MEDIO) scoreElement.style.color = '#fdcb6e';
              else scoreElement.style.color = '#ff7675';
            }
            ...
            state.interfaceUpdatePending = false;
          });
        }
        ...
        function adicionarCache(cache, valor, maxSize = 100) {
          cache.push(valor);
          if (cache.length > maxSize) cache.shift();
        }
        ...
        function calcularStochastic(highs, lows, closes, 
          ...
          try {
            ...
            for (let i = 0; i < periodoK; i++) {
              if (highs[i] > highestHigh) highestHigh = highs[i];
              if (lows[i] < lowestLow) lowestLow = lows[i];
            }
            
            // Calcular primeiro valor K
            kValues[0] = ((closes[periodoK - 1] - lowestLow) / (highestHigh - lowestLow)) * 100 || 50;
            
            // Calcular valores K com janela deslizante
            for (let i = periodoK; i < closes.length; i++) {
              const leavingHigh = highs[i - periodoK];
              const leavingLow = lows[i - periodoK];
              const newHigh = highs[i];
              const newLow = lows[i];
              
              // Atualizar extremos se necessário
              if (newHigh > highestHigh) {
                highestHigh = newHigh;
              } else if (leavingHigh === highestHigh) {
                highestHigh = Math.max(...highs.slice(i - periodoK + 1, i + 1));
              }
              
              if (newLow < lowestLow) {
                lowestLow = newLow;
              } else if (leavingLow === lowestLow) {
                lowestLow = Math.min(...lows.slice(i - periodoK + 1, i + 1));
              }
              
              kValues[i - periodoK + 1] = ((closes[i] - lowestLow) / (highestHigh - lowestLow)) * 100 || 50;
            }
          ...
          try {
            if (state.macdCache.emaRapida === null || state.macdCache.emaLenta === null) {
              ...
              state.macdCache = {
                ...
                macdLine: macdLinha.slice(-100),
                signalLine: sinalLinha.slice(-100)
              };
            ...
            adicionarCache(state.macdCache.macdLine, novaMacdLinha, 100);
            
            if (state.macdCache.signalLine.length === 0) {
              adicionarCache(state.macdCache.signalLine, novaMacdLinha, 100);
            } else {
              ...
              adicionarCache(state.macdCache.signalLine, novoSignal, 100);
            }
        ...
        function calcularSuperTrend(dados, periodo = CONFIG.PERIODOS.SUPERTREND, multiplicador = 3) {
          try {
            ...
            adicionarCache(state.superTrendCache, { direcao, valor: superTrend }, 100);
          ...
          try {
            ...
            if (priceHighs.length >= 2 && rsiHighs.length >= 2) {
              ...
              const validDivergence = 
                Math.abs(lastPriceHigh.value - prevPriceHigh.value) > (state.atrGlobal * 0.5) &&
                Math.abs(lastRsiHigh.value - prevRsiHigh.value) > 5;
              
              if (validDivergence && lastPriceHigh.value > prevPriceHigh.value && 
            ...
            if (priceLows.length >= 2 && rsiLows.length >= 2) {
              ...
              const validDivergence = 
                Math.abs(lastPriceLow.value - prevPriceLow.value) > (state.atrGlobal * 0.5) &&
                Math.abs(lastRsiLow.value - prevRsiLow.value) > 5;
              
              if (validDivergence && lastPriceLow.value < prevPriceLow.value && 
        ...
        async function analisarMercado() {
          ...
          try {
            ...
            const ema5 = calcularEMA(closes, CONFIG.PERIODOS.EMA_CURTA) || closes.slice(-1)[0];
            const ema13 = calcularEMA(closes, CONFIG.PERIODOS.EMA_MEDIA) || closes.slice(-1)[0];
            ...
            if (criteriosElement) {
              criteriosElement.innerHTML = `
                <li>Tendência: <span class="indicator-value">${state.tendenciaDetectada} (${state.forcaTendencia}%)</span></li>
                <li>Preço: <span class="indicator-value">${indicadores.close.toFixed(2)}</span></li>
                <li>RSI: <span class="indicator-value ${rsi < 30 ? 'positive' : rsi > 70 ? 'negative' : 'neutral'}">${rsi.toFixed(2)}</span> ${rsi < CONFIG.LIMIARES.RSI_OVERSOLD ? '🔻' : rsi > CONFIG.LIMIARES.RSI_OVERBOUGHT ? '🔺' : ''}</li>
                <li>MACD: <span class="indicator-value ${macd.histograma > 0 ? 'positive' : 'negative'}">${macd.histograma > 0 ? '+' : ''}${macd.histograma.toFixed(4)}</span> ${macd.histograma > 0 ? '🟢' : '🔴'}</li>
                <li>Stochastic: <span class="indicator-value">${stoch.k.toFixed(2)}/${stoch.d.toFixed(2)}</span></li>
                <li>Médias: <span class="indicator-value">EMA5 ${ema5.toFixed(2)} | EMA13 ${ema13.toFixed(2)}</span></li>
                <li>Suporte: <span class="indicator-value">${state.suporteKey.toFixed(2)}</span> | Resistência: <span class="indicator-value">${state.resistenciaKey.toFixed(2)}</span></li>
                <li>Divergência: <span class="indicator-value">${divergencias.tipoDivergencia}</span></li>
                <li>SuperTrend: <span class="indicator-value ${superTrend.direcao > 0 ? 'positive' : 'negative'}">${superTrend.direcao > 0 ? 'ALTA' : 'BAIXA'} (${superTrend.valor.toFixed(2)})</span></li>
                <li>Volatilidade (ATR): <span class="indicator-value">${atr.toFixed(4)}</span></li>
                <li>Lateral: <span class="indicator-value">${lateral ? 'SIM' : 'NÃO'}</span></li>
              `;
            }
        ...
        function sincronizarTimer() {
          ...
          state.timer = Math.floor((nextMinute - now) / 1000);
          
          const elementoTimer = document.getElementById("timer");
          if (elementoTimer) {
            elementoTimer.textContent = formatarTimer(state.timer);
            elementoTimer.style.color = state.timer <= 5 ? '#ff7675' : '#fdcb6e';
          }
